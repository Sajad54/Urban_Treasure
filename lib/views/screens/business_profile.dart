  ...
  Future<void> _fetchBusinessProfile() async {
    ...
    if (user != null) {
      final response = await Supabase.instance.client
          .from('profiles')
          .select('username, mfa_enabled, category, location')
          .eq('id', user.id)
          .maybeSingle();
      ...
      if (response != null && mounted) {
        setState(() {
          ...
          _selectedCategory = response['category'] ?? 'Food';
          _selectedLocation = response['location'] ?? 'Washington, DC';
        ...
        child: Column(
          ...
          children: [
            ...
            DropdownButton<String>(
              ...
              isExpanded: true,
              items: <String>['Washington, DC', 'Fairfax, VA', 'Frederick, MD']
                  .map((value) =>
                      DropdownMenuItem(value: value, child: Text(value)))
                  .toList(),
              onChanged: (value) async {
